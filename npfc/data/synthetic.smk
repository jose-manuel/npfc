...
natref_uni_reffile = config['natref_uni_reffile']
natref_fmap_dir = config['natref_fmap_dir']
...
INPUT_OUT = [WD + "/01_chunk/data/chembl_" + cid + ".sdf.gz" for cid in chunk_ids]  # chunk_sdf
...
rule PNP:
    priority: 0
    input: "{WD}/10_map/data/{prefix}_{cid}_map.csv.gz"
    output: "{WD}/11_pnp/data/{prefix}_{cid}_pnp.csv.gz"
    log: "{WD}/11_pnp/log/{prefix}_{cid}_pnp.log"
    shell: "fmaps_annotate_pnp {input} {natref_fmap_dir} {output} >{log} 2>&1"

rule MAP:
    priority: 1
    input: "{WD}/09_fcc/data/{prefix}_{cid}_fcc.csv.gz"
    output: "{WD}/10_map/data/{prefix}_{cid}_map.csv.gz"
    log: "{WD}/10_map/log/{prefix}_{cid}_map.log"
