...
def test_run():
    ...
    output_files = [f"tests/tmp/fc/03_natural/dnp/data/prep/frags_crms/08_fcg/data/dnp_{str(cid+1).zfill(3)}_fcg.csv.gz" for cid in range(2)]
    output_svg = 'tests/tmp/fc/03_natural/dnp/natural_dnp_tasktree.svg'
    # run protocol
    command_smk = 'run_protocol_fc natural -c fc/03_natural/dnp/test_natural_dnp_fragscrms.json'
