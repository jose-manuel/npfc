...
def test_find_fragments_hit_tautomers_1(df_mols_tautomers_1, df_frags_tautomers_1):
    ...
    # tautomer-dependant
    df_aidxf = fragment_search.get_fragment_hits(df_mols_tautomers_1, df_frags_tautomers_1)
    logging.debug(f"Fragment Hits:\n{df_aidxf}\n")
    assert len(df_aidxf) == 1
    assert list(df_aidxf['idm']) == ['TAUT_1A']
    # tautomer-independant
    df_aidxf = fragment_search.get_fragment_hits(df_mols_tautomers_1, df_frags_tautomers_1, tautomer=True)
    logging.debug(f"Fragment Hits:\n{df_aidxf}\n")
    assert len(df_aidxf) == 2
    assert list(df_aidxf['idm']) == ['TAUT_1A', 'TAUT_1B']


def test_find_fragments_hit_tautomers_2(df_mols_tautomers_2, df_frags_tautomers_2):
    """Check that tautomer-dependant search fails to identify both hits but tautomer-independant searchs succeeds (#2)"""
    # tautomer-dependant
    df_aidxf = fragment_search.get_fragment_hits(df_mols_tautomers_2, df_frags_tautomers_2)
    logging.debug(f"Fragment Hits:\n{df_aidxf}\n")
    assert len(df_aidxf) == 1
    assert list(df_aidxf['idm']) == ['TAUT_2B']
    # tautomer-independant
    df_aidxf = fragment_search.get_fragment_hits(df_mols_tautomers_2, df_frags_tautomers_2, tautomer=True)
    logging.debug(f"Fragment Hits:\n{df_aidxf}\n")
    assert len(df_aidxf) == 2
    assert list(df_aidxf['idm']) == ['TAUT_2A', 'TAUT_2B']


def test_find_fragments_hit_tautomers_3(df_mols_tautomers_3, df_frags_tautomers_3):
    """Check that tautomer-independant does not outperform tautomer-dependant on this particular case"""
    # tautomer-dependant
    df_aidxf = fragment_search.get_fragment_hits(df_mols_tautomers_3, df_frags_tautomers_3)
    logging.debug(f"Fragment Hits:\n{df_aidxf}\n")
    assert len(df_aidxf) == 3
    assert list(df_aidxf['idm']) == ['TAUT_3A', 'TAUT_3A', 'TAUT_3B']
    # tautomer-independant
    df_aidxf = fragment_search.get_fragment_hits(df_mols_tautomers_3, df_frags_tautomers_3, tautomer=True)
    logging.debug(f"Fragment Hits:\n{df_aidxf}\n")
    assert len(df_aidxf) == 3
    assert list(df_aidxf['idm']) == ['TAUT_3A', 'TAUT_3A', 'TAUT_3B']
